...
export function hasLessThanThreeTotalEventsRegistered(
  cartItems: CartItem[],
  events: { isRegistered: boolean }[]
): boolean {
  const alreadyRegisteredCount = events.filter((e) => e.isRegistered).length;
  const inCartCount = cartItems.filter((item) => item.type === "registration").length;

  return alreadyRegisteredCount + inCartCount < 3;
}
